...
export const NavigationButtons = ({ course, currentEpisode }) => {
  ...
  const currentIndex = allEpisodes.findIndex(ep => 
    ep.path === `/${course.id}/${currentEpisode.seasonId}/${currentEpisode.episodeId}`
  );
      ...
      {prevEpisode ? (
        ...
        >
          ...
          <div className="text-left">
            ...
            <p className="text-sm font-medium truncate max-w-48">{prevEpisode.title}</p>
          </div>
        </Link>
      ) : (
        <div></div>
      ...
      {nextEpisode ? (
        ...
        >
          <div className="text-right">
            ...
            <p className="text-sm font-medium truncate max-w-48">{nextEpisode.title}</p>
          </div>
